...
function crop_mouse_double_inv(nz, brainID, prefix)
% prefix = 'PMD';
% nz = 1;
% brainID = 'PMD3691&3692';
...
%% Invert Brain Clubbed ID
brainInvName = [prefix num2str(brain2) '&' num2str(brain1)];
...
for nD = 1 : length(direcBrain)
    try
        ...
        for i = 1:endCounter
            ...
            % Invert clubbed Name
            secNum = strrep(secNum, brainName, brainInvName);
            cmdROI = [cmdROI ' -roi "$OUTPUT_JP2_BASE_FOLDER/'];
            cmdROI = [cmdROI brainInvName '/' brainUP '/'];
        ...
        for i = 1:endCounter
            ...
            secNum = strrep(secNum, brainName, brainInvName);
            cmdROI = [cmdROI ' -roi "$OUTPUT_JP2_BASE_FOLDER/'];
            cmdROI = [cmdROI brainInvName '/' brainDN '/'];
            cmdROI = [cmdROI secNum];
            cmdROI = [cmdROI ','];
