...
% crop_marmoset(<int>nz, <string>brainID, <string>prefix)
% nz --> NanoZomer Number (1/2/3)
% Usage: crop_marmoset(1, 'MD915', 'MD')
% OP: in cwd .
%     MD915.sh
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


function crop_marmoset(nz, brainID, prefix)
%     prefix = 'MD';
%     nz = 3;
%     brainID = 'MD928';
...
for nD = 1 : length(direcBrain)
    try
        ...
        if species == 'Marmoset'
            ...
            for i = length(RP):-1:1
                if RP(i).Area < 1000
                    ...
                    RPM(cntM) = RP(i);
                    RPM(cntM).BoundingBox(1) = RPM(cntM).BoundingBox(1) - 10;
                    RPM(cntM).BoundingBox(2) = RPM(cntM).BoundingBox(2) - 10;
                    RPM(cntM).BoundingBox(3) = RPM(cntM).BoundingBox(3) + 20;
                    RPM(cntM).BoundingBox(4) = RPM(cntM).BoundingBox(4) + 20;
        ...
        for i = 1:endCounter
            ...
            if length(RPM)<2
                if RPM(i).BoundingBox(1)<600
                    nb = ((str2num(secName(secfind(3)+1:secfind(4)-1))-1)*2)+2;
                    ...
                    nb = ((str2num(secName(secfind(3)+1:secfind(4)-1))-1)*2)+1;
                    secNum_ad = compose('%04d', nb);
                    secNum = [strrep(secName(1:secfind(5)-1), ' ', '') ...
                        strrep(secName(secfind(5)+1:secfind(6)-1), '-', '.') ...
                ...
                nb = ((str2num(secName(secfind(3)+1:secfind(4)-1))-1)*2)+i;
                secNum_ad = compose('%04d', nb);
                secNum = [strrep(secName(1:secfind(5)-1), ' ', '') ...
                    strrep(secName(secfind(5)+1:secfind(6)-1), '-', '.') ...
