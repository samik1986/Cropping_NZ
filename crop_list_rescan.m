function crop_list_rescan(nz, brainID, prefix)
%     prefix = 'MD';
%     nz = 2;
%     brainID = 'MD850';
% clear all;
...
slideInfo = ['/nfs/data/qc/qcdisk005/NanoZoomer_' num2str(nz) '/' brainID '/slideInfoReScan.csv'];
...
for nD = 1 : length(direcBrain)
    try
        ...
        slideExists = ismember(slideMat{:,1}, slideNo);        
        tempSlideMat = slideMat(slideExists,:);        
        trcExists =  ismember(tempSlideMat{:,3}, tracerType);
        
        if trcExists
            
            noOfSlides = tempSlideMat{trcExists,4}-1;
